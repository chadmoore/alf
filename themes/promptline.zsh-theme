...
function __promptline_vpn_name {
  __name=$(cat $HOME/.current_vpn_name)
  if [ -z "$__name" ]; then
    vpn_name="🔓"
...
function __promptline_right_prompt {
  ...
  __promptline_wrapper "$(__promptline_vpn_name)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; }
...
function __promptline {
  ...
  local y_bg="${wrap}48;5;12${end_wrap}"
  local y_sep_fg="${wrap}38;5;12${end_wrap}"
  if [[ -n ${ZSH_VERSION-} ]]; then
